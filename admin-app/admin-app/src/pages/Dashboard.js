...
import { getMonthData, getOrders, getYearData } from '../features/auth/authSlice';
...
const Dashboard = () => {
  ...
  const monthDataState = useSelector(state => state?.auth?.monthData)
  const yearDataState = useSelector(state => state?.auth?.yearData)
  const orderState = useSelector(state => state?.auth?.orders?.orders)
  const [datamonth, setDatamonth] = useState([]);
  const [datamonthSales, setDatamonthSales] = useState([]);
  ...
  useEffect(()=>{
    ...
    for (let index = 0; index < monthDataState?.length; index++) {
      ...
      data.push({
        type:monthNames[element?._id?.month]?monthNames[element?._id?.month]:"January",
    ...
    setDatamonth(data)
    setDatamonthSales(monthOrderCount)
